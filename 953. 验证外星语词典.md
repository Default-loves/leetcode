...
复制数组words，自定义排序类排序复制的数据，然后对比原数组words和排序后的数据是否一致，如果不一致则返回false
...
class Solution {
    public boolean isAlienSorted(String[] words, String order) {
        ...
        String[] wordClone = Arrays.copyOf(words, words.length);
        Arrays.sort(wordClone, (a, b) -> {
            int i = 0, j = 0;
...
我们将自定义排序类修改为方法，每次对比words数组相邻的元素
